    ...
    <div className="flex md:flex-row flex-col items-center gap-6 justify-between">
      <div className="relative w-fit">
        <CustomImage
          size={100}
          avatar_url={testimonialData[1].avatar_url}
          className={`absolute md:top-[40px] md:left-[40px] top-[20px] left-[20px] md:w-fit w-[70px] z-10 ${
            animate ? "fade-in" : ""
          }`}
        />
        <CustomImage
          size={100}
          avatar_url={testimonialData[2].avatar_url}
          className={`absolute md:top-[40px] md:right-[40px] top-[20px] right-[20px] md:w-fit w-[70px] z-10 ${
            animate ? "fade-in" : ""
          }`}
        />
        <div className="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]">
          ...
          <CustomImage
            size={200}
            className={`md:w-full w-[100px] ${animate ? "fade-in" : ""}`}
            avatar_url={testimonialData[0].avatar_url}
          />
          <div className="bg-primary p-4 w-fit rounded-full absolute top-[50%] md:-right-10 -right-6 translate-y-[-50%]">
            <QuoteIcon className="md:w-12 md:min-h-12 w-6 min-h-6 fill-white text-white rotate-180" />
      ...
      <div className="md:max-w-[50%] max-w-full w-full flex flex-col justify-center items-center space-y-4">
        <h1
          className={`md:text-3xl text-2xl font-medium md:text-left text-center w-full ${
            animate ? "fade-in" : ""
          }`}
        >
          {data[index].title}
        ...
        <p
          className={`text-muted-foreground dark:text-white/70 md:text-lg text-sm text-justify min-h-[200px] ${
            animate ? "fade-in" : ""
          }`}
        >
          {data[index].description}
        ...
        <div className="flex gap-2 self-start">
          {testimonialImage.map((image, i) => (
            <Image
              ...
              className={cn(
                `object-cover aspect-square rounded-full cursor-pointer ease-linear transition-all duration-200`,
                data[index].avatar_url === image.avatar_url
                  ? "opacity-1 scale-[1.5] mx-2"
                  : "opacity-50"
              )}
        ...
        <p
          className={`md:text-lg text-base font-medium self-start mt-4 ${
            animate ? "fade-in" : ""
          }`}
        >
          {data[index].name}
